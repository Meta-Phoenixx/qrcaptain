...
function HealthDonut({ score, trend }: { score: number; trend?: number }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      <p className="text-[10px] uppercase tracking-widest text-white/40 font-medium mb-1">Fleet Health Score</p>
      <div className="relative">
        <svg width="120" height="120" viewBox="0 0 120 120">
          <circle cx="60" cy="60" r="46" fill="none" strokeWidth="10" stroke="rgba(255,255,255,0.06)" />
          {score > 0 && (
            <path
              d={arcPath(60, 60, 46, 0, end)}
              fill="none" stroke={color} strokeWidth="10"
              strokeLinecap="round"
              style={{ transition: "d 0.8s cubic-bezier(.4,0,.2,1)" }}
            />
          )}
...
function KpiTile({
  ...
  const colorMap = {
    ...
    red:    "text-red-400",
    blue:   "text-captain-400",
    gray:   "text-white/40",
  };
  return (
    <button
      onClick={onClick}
      className="flex flex-col items-center justify-center px-4 py-4 rounded-2xl bg-white/[0.04] border border-white/[0.07] hover:bg-white/[0.07] transition-colors text-center min-w-0"
    >
      <span className={`text-3xl font-bold tabular-nums leading-none ${colorMap[color]}`}>{value}</span>
      <span className={`text-[11px] font-medium mt-1 ${colorMap[color]}`}>{label}</span>
...
function FleetOverviewDonut({
  total, inService, inMaintenance, approaching, overdue,
  onFilter,
}: {
  total: number; inService: number; inMaintenance: number; approaching: number; overdue: number;
  onFilter: (f: string) => void;
}) {
  if (total === 0) return <div className="w-40 h-40 rounded-full bg-white/[0.04] flex items-center justify-center text-white/20 text-xs">No vessels</div>;
  ...
  const arcs = segments.map((seg) => {
    ...
    const path = arcPath(80, 80, 60, startDeg, startDeg + sweep);
    startDeg += sweep;
    return { ...seg, path };
  });
    ...
    <div className="flex items-center gap-6">
      <div className="relative flex-shrink-0">
        <svg width="160" height="160" viewBox="0 0 160 160">
          <circle cx="80" cy="80" r="60" fill="none" strokeWidth="16" stroke="rgba(255,255,255,0.05)" />
          {arcs.map((arc, i) => (
            <path key={i} d={arc.path} fill="none" stroke={arc.color} strokeWidth="16" strokeLinecap="butt" />
          ))}
          <text x="80" y="74" textAnchor="middle" fontSize="28" fontWeight="700" fill="white">{total}</text>
          <text x="80" y="90" textAnchor="middle" fontSize="9" fill="rgba(255,255,255,0.4)">TOTAL</text>
          <text x="80" y="102" textAnchor="middle" fontSize="9" fill="rgba(255,255,255,0.4)">VESSELS</text>
        </svg>
      </div>
    ...
    <div className="flex items-center gap-6">
      <div className="relative flex-shrink-0">
        <svg width="140" height="140" viewBox="0 0 140 140">
          <circle cx="70" cy="70" r="52" fill="none" strokeWidth="14" stroke="rgba(255,255,255,0.05)" />
          {overdue > 0 && (
            <path d={arcPath(70, 70, 52, 0, overdueEnd)} fill="none" stroke="#ef4444" strokeWidth="14" strokeLinecap="butt" />
          )}
          {approaching > 0 && (
            <path d={arcPath(70, 70, 52, overdueEnd, 359.99)} fill="none" stroke="#f97316" strokeWidth="14" strokeLinecap="butt" />
          )}
          <text x="70" y="62" textAnchor="middle" fontSize="30" fontWeight="700" fill="white">{total}</text>
...
// ─── Icon stat card (bottom row) ─────────────────────────────────────────────
function IconStat({
  icon, value, label, sub, linkLabel, onLink,
}: {
  ...
  return (
    <div className="flex flex-col gap-3 p-5 rounded-2xl bg-white/[0.03] border border-white/[0.07]">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-xl bg-captain-500/15 flex items-center justify-center flex-shrink-0">
          {icon}
        </div>
      ...
      <button onClick={onLink} className="text-[11px] text-captain-400 hover:text-captain-300 font-medium text-left transition-colors">
        {linkLabel} →
      </button>
    </div>
  );
...
function QuickActions({
  ...
  return (
    <div className="rounded-2xl bg-white/[0.03] border border-white/[0.07] overflow-hidden flex-shrink-0 w-64">
      ...
      <div className="p-3 flex flex-col gap-2">
        <button
          onClick={onAddVessel}
          className="w-full flex items-center gap-2 px-4 py-2.5 rounded-xl bg-captain-500 hover:bg-captain-400 text-white text-sm font-semibold transition-colors"
        >
        ...
        {actions.map((a) => (
          <button
            key={a.label}
            onClick={a.action}
            className="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl hover:bg-white/[0.05] text-white/60 hover:text-white/90 text-sm transition-colors text-left"
          >
        ...
        <div className="border-t border-white/[0.06] mt-1 pt-2">
          <button className="w-full text-xs text-captain-400 hover:text-captain-300 py-1.5 transition-colors font-medium">
            View All Actions →
          </button>
        </div>
      </div>
...
function FleetDropdown({
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/[0.05] border border-white/[0.08] hover:bg-white/[0.08] transition-colors text-sm"
      >
        <span className="font-semibold text-white">{selected?.name ?? "Select Fleet"}</span>
        <span className="text-white/40 text-xs">↓</span>
        {selected && <span className="text-white/40 text-xs">{selected.vesselCount} Vessels</span>}
      </button>
      {open && (
        <div className="absolute right-0 top-full mt-1 w-52 rounded-xl bg-[#1a2540] border border-white/[0.1] shadow-xl z-50 overflow-hidden">
          {fleets.map((f) => (
            <button
              key={f._id}
              onClick={() => { onSelect(f._id); setOpen(false); }}
...
export function FleetDashboard() {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen px-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-white/50 mb-6">Create your first fleet to start tracking vessels, managing mechanics, and monitoring service across your entire operation.</p>
  ...
  return (
    <div className="min-h-screen px-6 py-6 space-y-5 max-w-[1400px] mx-auto">

      {/* ── Page header ──────────────────────────────────────────────────── */}
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold font-heading text-white">Fleet Command Center</h1>
          <p className="text-sm text-white/40 mt-0.5">Real-time health and service status across your fleet</p>
        </div>
        <div className="flex items-center gap-3">
          {/* Notification bell */}
          <button className="relative p-2.5 rounded-xl bg-white/[0.04] border border-white/[0.08] hover:bg-white/[0.08] transition-colors text-white/50 hover:text-white">
          ...
          {selectedFleetId && (
            <button
              ...
              className="p-2.5 rounded-xl bg-white/[0.04] border border-white/[0.08] hover:bg-white/[0.08] transition-colors text-white/50 hover:text-white"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-captain-500 hover:bg-captain-400 text-white text-sm font-semibold transition-colors"
          >
            + New Fleet
          </button>
        </div>
      ...
      {/* ── Top KPI strip ────────────────────────────────────────────────── */}
      <div className="grid grid-cols-[200px_1fr_1fr_1fr_1fr_1fr_auto] gap-3 items-stretch">
        {/* Health score */}
        <div className="flex items-center justify-center p-4 rounded-2xl bg-white/[0.04] border border-white/[0.07]">
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="rounded-2xl bg-white/[0.03] border border-white/[0.07] p-5 flex flex-col gap-4">
              ...
              <FleetOverviewDonut
                total={d.totalVessels}
                inService={d.inServiceCount}
                inMaintenance={d.inMaintenanceCount}
                approaching={d.approachingCount}
                overdue={d.overdueCount + d.outOfServiceCount}
                onFilter={setActiveFilter}
              />
              <button
                onClick={() => setActiveFilter(null)}
                className="text-xs text-captain-400 hover:text-captain-300 font-medium transition-colors text-left mt-auto"
              >
                View All Vessels →
              </button>
            </div>
            ...
            <div className="rounded-2xl bg-white/[0.03] border border-white/[0.07] p-5 flex flex-col gap-4">
              ...
              <div className="flex h-2 rounded-full overflow-hidden gap-0.5">
                <div className="flex-1 bg-emerald-500 rounded-l-full" style={{ flex: d.inServiceCount }} />
                <div className="flex-1 bg-amber-500" style={{ flex: d.inMaintenanceCount }} />
                <div className="flex-1 bg-red-500 rounded-r-full" style={{ flex: Math.max(d.outOfServiceCount + d.overdueCount, 0.5) }} />
              </div>
          ...
          <div className="grid grid-cols-5 gap-3">
            ...
            <IconStat
              ...
              value={d.totalPartsFlagged}
              label="Parts Flagged"
              sub="Requiring attention"
              linkLabel="View Parts List"
              onLink={() => router.push("/vessels")}
            />
            <IconStat
              ...
              value={d.avgEngineHours != null ? d.avgEngineHours.toLocaleString() : "—"}
              label="Engine Hours"
              sub="Avg hours per vessel"
              linkLabel="View Engine Hours"
              onLink={() => router.push("/vessels")}
            />
            <IconStat
              icon={<svg className="w-5 h-5 text-captain-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg>}
              value={`${d.coveredVessels}/${d.totalVessels}`}
              label="Mechanic Coverage"
              sub="Vessels assigned"
              linkLabel="View Assignments"
              onLink={() => setShowSettings(true)}
            />
            <IconStat
              icon={<svg className="w-5 h-5 text-captain-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>}
              value={d.warrantyExpiringSoon}
