...
const NAV_ITEMS: NavItem[] = [
  ...
  },
  {
    label: "Work Orders",
    href: "/work-orders",
    roles: ["fleet_manager", "mechanic", "admin"],
    exact: true,
    icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>,
  },
  // ── Directory ─────────────────────────────────────────────────────────
  ...
  // ── Operations ────────────────────────────────────────────────────────
  {
    label: "Maintenance",
    href: "/maintenance",
    roles: ["owner", "fleet_manager", "admin"],
  ...
  },
  {
    ...
    href: "/parts",
    roles: ["owner", "fleet_manager", "mechanic", "admin"],
...
export function AppSideNav() {
  ...
  const role = (me?.role ?? "owner") as Role;
        ...
        <div className="relative" ref={profileRef}>
            ...
            <div className="absolute bottom-full left-0 right-0 mb-2 rounded-xl bg-[#1a2540] border border-white/[0.1] shadow-xl overflow-hidden z-50">
              <div className="px-4 py-3 border-b border-white/[0.08]">
                <p className="text-xs font-semibold text-white truncate">
                  {me ? `${me.firstName ?? ""} ${me.lastName ?? ""}`.trim() || me.name || "" : ""}
                </p>
          ...
          >
            ...
            <div className="min-w-0 flex-1 text-left">
              <p className="text-xs font-semibold text-white truncate">
                {me ? `${me.firstName ?? ""} ${me.lastName ?? ""}`.trim() || me.name || "User" : "Loading…"}
              </p>
