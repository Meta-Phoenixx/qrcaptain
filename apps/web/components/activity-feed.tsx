...
export function ActivityFeed({ maxItems = 10, onViewWorkOrder, onViewQuote }: ActivityFeedProps) {
  ...
  const getActivityColor = (type: string) => {
    if (type.includes("quote")) return "bg-blue-100 text-blue-600";
    if (type.includes("work_order")) {
      if (type.includes("completed")) return "bg-green-100 text-green-600";
      if (type.includes("started")) return "bg-yellow-100 text-yellow-600";
      return "bg-purple-100 text-purple-600";
    }
    if (type.includes("message")) return "bg-indigo-100 text-indigo-600";
    if (type.includes("access")) return "bg-orange-100 text-orange-600";
    if (type.includes("rating")) return "bg-amber-100 text-amber-600";
      ...
      {displayedActivities.map((activity) => (
        <button
          ...
          className={`w-full flex items-start gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors text-left ${
            !activity.isRead ? "bg-captain-50/50" : ""
          }`}
        >
          {/* Icon */}
          ...
          {/* Unread indicator */}
          {!activity.isRead && (
            <div className="flex-shrink-0 w-2 h-2 bg-captain-600 rounded-full mt-2" />
          )}
        </button>
      ))}
...
export function ActivitySummary() {
  ...
  if (!notifications) {
    return <div className="h-4 bg-gray-100 rounded w-24 animate-pulse" />;
  }
  ...
  return (
    <div className="flex items-center gap-4 text-sm">
      {unreadCount > 0 && (
        <span className="flex items-center gap-1 text-captain-600">
          <span className="w-2 h-2 bg-captain-600 rounded-full" />
          {unreadCount} unread
        </span>
      )}
      <span className="text-gray-500">{recentCount} in last 24h</span>
    </div>
  );
}
