...
function ServicePill({ isOverdue, isApproaching }: { isOverdue?: boolean; isApproaching?: boolean }) {
  if (isOverdue) return (
    <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-red-500/15 text-red-400">⚠ Overdue</span>
  );
  if (isApproaching) return (
    <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-amber-500/15 text-amber-400">⏱ Due Soon</span>
  );
  return (
    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-emerald-500/15 text-emerald-400">✓ Current</span>
  );
}
...
function NextServiceCell({ date }: { date: number | null | undefined }) {
  ...
  const color = days < 0 ? "text-red-400" : days <= 14 ? "text-amber-400" : "text-emerald-400";
  return (
    <div>
      <p className="text-xs text-white/60">{label}</p>
      <p className={`text-[10px] font-medium ${color}`}>{days < 0 ? `${Math.abs(days)}d overdue` : `in ${days} days`}</p>
    </div>
  );
}

function VesselThumb({ imageUrl, name }: { imageUrl?: string | null; name: string }) {
  if (imageUrl) {
    return (
  ...
  return (
    <div className="w-11 h-11 rounded-xl bg-captain-500/10 border border-captain-500/20 flex items-center justify-center flex-shrink-0 text-xl">
      ⛵
    </div>
  );
}

function MechanicCell({ name, company, photoUrl }: { name?: string | null; company?: string | null; photoUrl?: string | null }) {
  if (!name) return (
    <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs bg-amber-500/10 text-amber-400">Uncovered</span>
  );
  const initials = name.split(" ").map((w) => w[0]).join("").slice(0, 2).toUpperCase();
  return (
    <div className="flex items-center gap-2">
      <div className="w-8 h-8 rounded-full overflow-hidden flex-shrink-0 border border-captain-500/30">
        {photoUrl ? (
          <img src={photoUrl} alt={name} className="w-full h-full object-cover" />
        ) : (
          <div className="w-full h-full bg-captain-500/20 flex items-center justify-center">
            <span className="text-[10px] font-bold text-captain-300">{initials}</span>
          </div>
        )}
      </div>
...
export function FleetVesselTable({
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      rows = rows.filter((v) => v.name.toLowerCase().includes(q) || v.make?.toLowerCase().includes(q) || v.model?.toLowerCase().includes(q));
    }
  ...
  return (
    <div className={`rounded-2xl border overflow-hidden ${card}`}>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={8} className="px-5 py-12 text-center text-sm text-white/25">No vessels match this filter.</td>
              </tr>
            ) : (
              filtered.map((vessel) => (
                <tr key={vessel.vesselId} onClick={() => router.push(`/vessel/${vessel.vesselId}`)}
                  className={`border-b last:border-0 ${divider} ${rowHover} transition-colors cursor-pointer`}>
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-3">
                      <VesselThumb imageUrl={vessel.vesselImageUrl} name={vessel.name} />
                      <div>
                        <p className="text-sm font-semibold text-white">{vessel.name}</p>
                        <p className="text-[11px] text-white/40 mt-0.5">{[vessel.make, vessel.model].filter(Boolean).join(" ")}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3.5"><StatusPill status={vessel.status} /></td>
                  <td className="px-4 py-3.5"><ServicePill isOverdue={vessel.isOverdue} isApproaching={vessel.isApproaching} /></td>
                  <td className="px-4 py-3.5">
                    {vessel.currentEngineHours != null ? (
                      <div>
                        <span className="text-sm font-semibold text-white/80 tabular-nums">{vessel.currentEngineHours.toLocaleString()} hrs</span>
                        {vessel.engineManufacturer && <p className="text-[10px] text-white/35 mt-0.5">{vessel.engineManufacturer}</p>}
                      </div>
                    ) : <span className="text-xs text-white/25">—</span>}
                  </td>
                  <td className="px-4 py-3.5">
                    <MechanicCell name={vessel.mechanicName} company={vessel.mechanicCompany} photoUrl={vessel.mechanicPhotoUrl} />
                  </td>
                  <td className="px-4 py-3.5"><NextServiceCell date={vessel.nextServiceDate} /></td>
                  <td className="px-4 py-3.5">
      ...
      <div className={`px-5 py-3 border-t ${divider} flex justify-center`}>
        <button onClick={() => router.push("/vessels")} className="text-xs text-captain-400 hover:text-captain-300 font-medium transition-colors">
