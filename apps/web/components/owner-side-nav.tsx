...
const NAV_ITEMS = [
  ...
  },
  {
    ...
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M12 3a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm0 4v2M8 9h8M12 11v10M6 21c0-3.31 2.69-6 6-6s6 2.69 6 6" />
      </svg>
    ),
  },
  {
  ...
  return (
    <aside className="hidden lg:flex flex-col w-56 xl:w-60 flex-shrink-0 sticky top-0 h-screen bg-[#0d1526] border-r border-white/[0.06]">
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        {NAV_ITEMS.map((item) => {
          const exactOnly = ["/my-dashboard", "/my-vessels", "/my-work-orders"];
          const active =
            pathname === item.href ||
            (!exactOnly.includes(item.href) && pathname.startsWith(item.href));
          return (
            <button
              key={item.href}
